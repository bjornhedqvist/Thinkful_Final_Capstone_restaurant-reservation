  ...
  try {
    if (req.body.data.status && req.body.data.status !== "booked") {
      const error = new Error(
        `status must be "booked", received: ${req.body.data.status}`
      );
      error.status = 400;
      throw error;
    }
...
function hasStatusProperties(req, res, next) {
  try {
    if (
      ...
      const error = new Error(
        `Error, status is unknown, recieved: '${req.body.data.status}'. Expected: booked, seated, or finished.`
      );
      error.status = 400;
      throw error;
    }
    if (res.locals.reservation.status == "finished") {
      const error = new Error(
        `Error, status is currently '${res.locals.reservation.status}'. A 'finished' reservation cannot be updated.`
      );
      error.status = 400;
      throw error;
    }
